...
import time
from PII_data_processor import import_dataset, initialize_lists, stem_restricted, word_match_stemming, fuzzy_partial_stem_match, unique_entries, date_detection #,reviewed_potential_pii, recode, export, log
...
# def print(the_message):  # ensures compatibility in iPython notebook as well as printing to screen in GUI
    ...
    try:
        ...
        def evaluate(event=None):
            if entry.get() in ['y', 'yes']:
                return True
            res.configure(text="Ergebnis: " + str(entry.get()))
